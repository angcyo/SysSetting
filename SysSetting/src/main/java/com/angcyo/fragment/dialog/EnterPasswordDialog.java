...
import com.angcyo.fragment.adapter.node.WifiConnectInfo;
import com.angcyo.fragment.adapter.node.WifiStaticIpNode;
import com.angcyo.fragment.port.OnWifiConnect;
 ...
 */
public class EnterPasswordDialog extends Dialog implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public EnterPasswordDialog(Context context, int theme, OnWifiConnect listener, WifiConnectInfo info) {
        super(context, theme);
                ...
                if (password == null || password.length() == 0 || password.length() < 8) {
                    ...
                    if (cbStaticIp.isChecked()) {
                        ...
                        if (!node.ip.equalsIgnoreCase("0.0.0.0")) {
                            connect.onConnectStaticIp(wifiInfo.SSID, etWifiPassword.getText().toString(), wifiInfo.security, node);
                        } else {
                            connect.onConnect(wifiInfo.SSID, etWifiPassword.getText().toString(), wifiInfo.security);
                        }
                    } else {
                        connect.onConnect(wifiInfo.SSID, etWifiPassword.getText().toString(), wifiInfo.security);
                    }
    ...
    private WifiStaticIpNode getWifiStaticIpNode() {
        ...
        //保存配置
        XmlSetting.set(new String[]{XmlSetting.XML_STATIC_IP, XmlSetting.XML_STATIC_GATEWAY, XmlSetting.XML_STATIC_DNS},
                ip, gateway, dns);
        return node;
    }
    ...
    void showStaticIpLayout() {
        ...
        //获取配置
        Object[] values = XmlSetting.get(new String[]{XmlSetting.XML_STATIC_IP, XmlSetting.XML_STATIC_GATEWAY, XmlSetting.XML_STATIC_DNS});
