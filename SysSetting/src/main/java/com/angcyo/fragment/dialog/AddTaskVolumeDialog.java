...
import com.angcyo.db.helper.VolumeHelper;
 ...
 */
public class AddTaskVolumeDialog extends BaseTaskDialog implements View.OnClickListener {
    ...
    public AddTaskVolumeDialog(Context context, int theme, VolumeTaskRecord task, OnTaskAdd listener) {
        super(context, theme);
        this.taskAddListener = listener;
        this.taskRecord = task;
    }

    protected AddTaskVolumeDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.id_bt_volume_value_add:
                discreteSeekBar.setProgress(discreteSeekBar.getProgress() + 1);
                showFloater();
                break;
            case R.id.id_bt_volume_value_minus:
                discreteSeekBar.setProgress(discreteSeekBar.getProgress() - 1);
                showFloater();
                break;
    ...
    void asyncSaveTask(final boolean cancel) {
        new AsyncRun() {
            @Override
            public void doBack() {
                VolumeHelper.save(getHour(), getMinute(), getVolumeValue(), getPeriod(), STATE_ENABLED, new LocalDateTime().toString("yyyy-MM-dd HH:mm:ss"));
            }
